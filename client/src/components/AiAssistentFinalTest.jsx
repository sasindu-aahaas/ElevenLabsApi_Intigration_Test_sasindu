...
export default function AiAssistentFinalTest() {
  const [callId, setCallId] = useState("");
  const [callStatus, setCallStatus] = useState("Ready to start AI ASSISTENT FINAL TEST.");
  ...
  useEffect(() => {
    if (phase === "ringing" || phase === "connecting") {
      // Call starting — wake up the music very quietly in background
      initAmbientMusic();
      setAmbientVolume(0.06, 2.5);
    } else if (phase === "processing") {
      // AI is thinking — bring music up gradually
      initAmbientMusic();
      setAmbientVolume(0.32, 4.0);
    } else if (phase === "assistant-speaking") {
      // Agent speaking — duck the music way down so voice is clear
      setAmbientVolume(0.05, 1.2);
    } else if (phase === "listening") {
      // Mic is on — silence the music completely so it doesn't bleed in
      setAmbientVolume(0, 0.5);
    } else if (phase === "idle" || phase === "completed" || phase === "ending") {
      // Call ended — fade out and clean up
      destroyAmbientMusic();
    }
  ...
  // ── Ambient music (real MP3, looped, volume-controlled via GainNode) ──────────

  function initAmbientMusic() {
    if (ambientMusicRef.current) return; // already running — just adjust volume
  ...
  function resetState() {
    ...
    setCallId("");
    setCallStatus("Ready to start AI ASSISTENT FINAL TEST.");
    setPhase("idle");
    setError("");
    setConversation([]);
    setLastTranscript("");
  ...
  async function handleStartCall() {
    ...
    try {
      ...
      setPhase("connecting");
      setCallStatus("Connecting you to the AI ASSISTENT FINAL TEST receptionist now...");
      playConnectTone();
      ...
      if (!response.ok) throw new Error(data.message || "AI ASSISTENT FINAL TEST session could not start.");
  ...
  async function sendTurn(audioBlob, transcriptText = "") {
    try {
      ...
      const formData = new FormData();
      formData.append("call_id", callIdRef.current);
      if (audioBlob) formData.append("audio", audioBlob, "ai-assistent-final-test.webm");
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.message || "AI ASSISTENT FINAL TEST reply failed.");
    ...
    <div className="panel reception-panel">
      ...
      {!callSupported ? (
        <p className="error-text">
          This browser does not support microphone recording for AI ASSISTENT FINAL TEST.
        </p>
      ) : null}

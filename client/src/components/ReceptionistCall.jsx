...
function formatDebugJson(value) {
  if (!value) {
    return "No package suggested yet.";
  }
...
function inferListeningProfile(questionText) {
  ...
  if (normalized.includes("full name") || normalized.includes("your full name")) {
    return {
      ...
      hint: "Mic ready. Short answer expected, about 4 seconds."
    };
  }
  ...
  if (normalized.includes("contact number") || normalized.includes("phone")) {
    return {
      ...
      hint: "Mic ready. Say the phone number clearly."
    };
  }
  ...
  if (normalized.includes("email")) {
    return {
      ...
      hint: "Mic ready. Say the email slowly."
    };
  }
  ...
  if (normalized.includes("country") || normalized.includes("location")) {
    return {
      ...
      hint: "Mic ready. Short location answer expected."
    };
  }
  ...
  if (normalized.includes("booking id") || normalized.includes("reference number")) {
    return {
      maxRecordMs: 8000,
      postSpeechSilenceMs: 1800,
      hint: "Mic ready. Say the booking ID clearly."
    };
  }
  ...
  if (
    ...
    return {
      maxRecordMs: 5000,
      postSpeechSilenceMs: 1500,
      hint: "Mic ready. Say yes, or explain what to change."
    };
  }
  ...
  if (
    ...
    return {
      maxRecordMs: 12000,
      postSpeechSilenceMs: 2200,
      hint: "Mic ready. You have more time for detailed changes."
    };
  }
  ...
  if (
    ...
    return {
      maxRecordMs: 8000,
      postSpeechSilenceMs: 1800,
      hint: "Mic ready. Please answer in one short sentence."
    };
  }
  ...
  return {
    maxRecordMs: 7000,
    postSpeechSilenceMs: 1800,
    hint: "Mic ready. Please answer now."
  };
}

export default function ReceptionistCall() {
  ...
  async function handleStartCall() {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/reception-call/session`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Reception call session could not start.");
      }
  ...
  async function beginListening() {
    if (!callId || !autoLoopEnabledRef.current || finalizingRef.current) {
      return;
    }

    try {
      const profile = inferListeningProfile(lastReply);
      setPhase("listening");
      ...
      recorder.onstop = async () => {
        ...
        if (audioBlob.size === 0 || !autoLoopEnabledRef.current) {
          return;
        }
  ...
  async function sendTurn(audioBlob, transcriptText = "") {
    try {
      ...
      if (audioBlob) {
        formData.append("audio", audioBlob, "reception-call.webm");
      }
      ...
      const response = await fetch(`${API_BASE_URL}/reception-call/turn`, {
        method: "POST",
        body: formData
      });
      ...
      if (!response.ok) {
        throw new Error(data.message || "Reception call reply failed.");
      }
      ...
      stopHoldAudioLoop();
      setError(err.message);
      setCallStatus("The call could not continue. You can reset and start again.");
      setPhase("idle");
      autoLoopEnabledRef.current = false;
    }
  }
  ...
  async function handleSendTestMessage() {
    if (!callId || !testMessage.trim() || finalizingRef.current || callEnded) {
      return;
    }
  ...
  async function finalizeCall(endedReason = "completed") {
    if (!callId || finalizingRef.current) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/reception-call/end`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (!response.ok) {
        throw new Error(data.message || "Call report could not be created.");
      }
  ...
  return (
    <div className="panel reception-panel">
      ...
      <div className="panel-header reception-header">
        <div>
          <p className="eyebrow">Premium Voice Agent</p>
          ...
          <p className="panel-copy reception-copy">
            ...
            automatically, can be tested with a small text box, and ends by saving a structured service report.
          </p>
        </div>
        <span className="call-badge reception-badge">Auto call flow</span>
      </div>
        ...
        <div className="conversation-log reception-log" aria-live="polite">
          {conversation.length === 0 ? (
            ...
            conversation.map((message, index) => (
              <article
                key={`${message.role}-reception-${index}`}
